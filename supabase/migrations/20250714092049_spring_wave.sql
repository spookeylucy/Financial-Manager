...
-- Financial Goals
CREATE POLICY "Read own goals" ON financial_goals
  FOR SELECT TO authenticated
  USING (auth.uid() = user_id);

CREATE POLICY "Insert own goals" ON financial_goals
  FOR INSERT TO authenticated
  WITH CHECK (auth.uid() = user_id);

CREATE POLICY "Update own goals" ON financial_goals
  FOR UPDATE TO authenticated
  USING (auth.uid() = user_id);

CREATE POLICY "Delete own goals" ON financial_goals
  FOR DELETE TO authenticated
  USING (auth.uid() = user_id);
